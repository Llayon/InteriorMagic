...
export function Room(){const room=useEditorStore(s=>s.project.room),finishes=useEditorStore(s=>s.project.finishes);return <group>
  ...
  <mesh position={[0,.06,-room.depth/2+.015]}><boxGeometry args={[room.width,.12,.03]}/><meshStandardMaterial color="#eee9df" roughness={.9}/></mesh>
  <mesh position={[-room.width/2+.015,.06,0]}><boxGeometry args={[.03,.12,room.depth]}/><meshStandardMaterial color="#eee9df" roughness={.9}/></mesh>
