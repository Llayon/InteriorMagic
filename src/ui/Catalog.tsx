...
export function Catalog() {
  ...
  return <section className="sheet">
    ...
    <nav className="categories">{tabs.map(([id, label]) => <button key={id} className={category === id ? 'active' : ''} onClick={() => useEditorStore.getState().setCatalogCategory(id)}>{label}</button>)}</nav>
    {error && <div className="catalog-error" role="alert">{error}</div>}
    <div className="items">{assetList.filter((asset) => asset.category === category).map((asset) => <button className="item" key={asset.id} aria-busy={loadingAssetId === asset.id} onClick={() => void addAsset(asset)}>{asset.thumbnailUrl ? <img src={asset.thumbnailUrl} alt="" loading="lazy" /> : <span>{asset.icon}</span>}<b>{asset.name}</b><small>{loadingAssetId === asset.id ? 'Загрузка…' : 'Добавить'}</small></button>)}</div>
  </section>;
}
