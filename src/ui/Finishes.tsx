...
function Swatches({ kind, list, active, apply }: { kind: 'floor' | 'wall'; list: string[][]; active: string; apply: (id: string) => void }) {
  return <div className="swatches">{list.map(([id, color]) => <button key={id} data-finish-id={`${kind}:${id}`} aria-label={`${kind} finish ${id}`} className={active === id ? 'active' : ''} style={{ background: color }} onClick={() => apply(id!)} />)}</div>;
}
