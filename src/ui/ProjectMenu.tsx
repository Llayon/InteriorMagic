...
import { requestExplicitSave } from '@/editor/persistence/projectSyncController';

export function ProjectMenu({ disabled = false }: { disabled?: boolean } = {}) {
  const [open, setOpen] = useState(false);
  const [confirmReset, setConfirmReset] = useState(false);
  return <div className="project-menu" data-planner-locked={disabled ? 'on' : 'off'}><button className="round glass" aria-label="Project menu" disabled={disabled} onClick={() => setOpen((value) => !value)}>•••</button>{!disabled && open && <div className="project-pop glass"><button aria-label="Save project" onClick={() => { useEditorStore.getState().save(); void requestExplicitSave(); setOpen(false); }}>Сохранить</button><button aria-label="Load project" onClick={() => { useEditorStore.getState().load(); setOpen(false); }}>Загрузить</button><button aria-label="Reset project" className="danger-text" onClick={() => { setOpen(false); setConfirmReset(true); }}>Сбросить комнату</button></div>}
